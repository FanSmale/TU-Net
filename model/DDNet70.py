...
class DDNet70Model(nn.Module):
    def __init__(self, n_classes, in_channels, is_deconv, is_batchnorm):
        ...
        self.pre_seis_conv = SeismicRecordDownSampling(self.in_channels) # 下采样层，用于处理地震记录
    ...
    def forward(self, inputs, _=None):
        ...
        compress_seis = self.pre_seis_conv(inputs) # 输入图像通过网络预降维层
...
class SDNet70Model(nn.Module):
    def __init__(self, n_classes, in_channels, is_deconv, is_batchnorm):
        ...
        :param n_classes:    Number of channels of output (any single decoder) 输出通道的数量
        :param in_channels:  Number of channels of network input 输入图像的通道数
        :param is_deconv:    Whether to use deconvolution 是否使用反卷积进行上采样
        :param is_batchnorm: Whether to use BN 是否在每个卷积层之后使用批量归一化
        '''
        ...
        self.pre_seis_conv = SeismicRecordDownSampling(self.in_channels) # 预网络降维器
